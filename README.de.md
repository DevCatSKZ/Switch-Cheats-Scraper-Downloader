...
Zum Desktop-Tool gibt es ein **vollwertiges Gegenstück, das direkt auf der Switch läuft** — dieselbe Holo-Glass-App **auf der Konsole**, nicht bloß ein Downloader. Die eigenständige Homebrew-`.nro` (`SwitchCheatsDownloader.nro`) hat eine **Spielebibliothek auf der Konsole** (~3000 Spiele, SQLite) mit Cover-Galerie, Suche & Filtern, einem **Cheat-Editor**, Community-Quellen, der **CheatSlips-API** und einer **System-Seite** — installierte & laufende Spiele mit ihrer echten **Build-ID** sowie **Save-Backup & -Restore**. Sie lädt das stets aktuelle [`data`-Release](https://github.com/DevCatSKZ/Switch-Cheats-Scraper-Downloader/releases/tag/data) **auf der Konsole** und entpackt es direkt im Atmosphère-Format auf die SD-Karte — ganz ohne PC.

<p align="center"><img src="screenshots/switch-app.png" width="720" alt="Switch Cheats Downloader Homebrew-App"></p>

**Was sie macht:** Verwalte deine ganze Cheat-Bibliothek **auf der Konsole** — suchen, filtern, ein Spiel öffnen und seine Builds sehen, Cheat-Codes ansehen/bearbeiten, aus den Community-Quellen & CheatSlips laden, und die **System-Seite** nutzen (installierte/laufende Spiele mit Build-ID, Save-Backup/-Restore). Oder einfach alles holen: Ein Druck auf **A** lädt die neuesten Cheats und entpackt sie nach `atmosphere/contents/<TitleID>/cheats/` (abgebrochene Downloads werden **fortgesetzt**; dieselbe Erkennung „erneuter Upload ohne Versionssprung“ wie das Desktop-Tool). Die App kann sich **selbst aktualisieren** und spricht **6 Sprachen** (EN/DE/ES/FR/IT/JA, automatisch von der Konsole erkannt) — bedienbar per Joy-Con **und** Touch.
...
Derselbe Downloader läuft auch auf **Android**, für die Switch-Emulatoren **Eden, Suyu und Sudachi** (`SwitchCheatsDownloader-Android.apk`). Er holt das stets aktuelle [`data`-Release](https://github.com/DevCatSKZ/Switch-Cheats-Scraper-Downloader/releases/tag/data) **auf dem Handy**, entpackt die Cheats in einen öffentlichen Ordner, und du importierst sie pro Spiel über **Add-ons → Mods and cheats** des Emulators — ganz ohne PC.

<p align="center"><img src="screenshots/android-app.png" width="340" alt="Switch Cheats Downloader (Android)"></p>

**Was sie macht:** Tippe auf **Herunterladen & entpacken** — die App lädt alle Cheats und entpackt sie in einen öffentlichen Ordner als `.../SwitchCheats/<TitleID>/<Spielname>/cheats/<BuildID>.txt`, jeder Ordner nach dem echten Spielnamen benannt (im `switch-cheats-emulator.zip`-Paket bereits enthalten). Dann drückst du in **Eden, Suyu oder Sudachi** lange auf das Spiel → **Add-ons → „Mods and cheats“** → wählst den passenden Ordner, und der Emulator kopiert sie in seinen eigenen Speicher. Abgebrochene Downloads werden **fortgesetzt**, die App prüft dieses Repo auf **Updates** (eine neuere APK *und* frische Cheats, gleiche Erkennung wie im Desktop-Tool), zeigt den Online-Status live an und spricht dieselben **6 Sprachen** (EN/DE/ES/FR/IT/JA, automatisch erkannt). Gleicher **Holo-Glass**-Look wie die Windows- und Switch-App.

**Warum ein öffentlicher Ordner?** Ab Android 11 kann keine Drittanbieter-App in den privaten `Android/data/…`-Ordner eines Emulators schreiben — weder „Zugriff auf alle Dateien“ noch der Ordner-Picker kommen dort hin. Deshalb entpackt die App in einen **öffentlichen** Ordner (Standard `/storage/emulated/0/SwitchCheats`, änderbar), und der Emulator importiert von dort selbst. Das schnelle Schreiben vieler kleiner Dateien braucht „**Zugriff auf alle Dateien**“ (`MANAGE_EXTERNAL_STORAGE`), das die App ab Android 11 einmalig anfragt.
...
- **HTML-Cheat-Anzahl aus der „Game releases"-Tabelle**: Die Spalte „Available cheats" wird jetzt direkt ausgelesen, sodass ein Build seine echte Cheat-Zahl zeigt — auch wenn die API diesen Build nicht mitliefert (statt fälschlich 0).- **Voll gepflegte SQLite-Datenbank** (`cheats.db`) mit allen Feldern: Name, Title-ID, Build-ID, Version, Upload-Datum, Cheat-Namen, Credits, Beschreibung, Cover-Bild, Banner, **Region** (US/EU/AU/JP/KR/HK/CN/Homebrew) und **Quelle**.
