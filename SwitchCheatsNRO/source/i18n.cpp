...
const Entry kTable[] = {
    ...
    {"footer.nav",     {"L/R Seite   Hoch/Runter Wählen   A OK   B Zurück   + Beenden", "L/R Page   Up/Down Select   A OK   B Back   + Exit", "L/R Página   Arriba/Abajo Elegir   A OK   B Volver   + Salir", "L/R Page   Haut/Bas Choisir   A OK   B Retour   + Quitter", "L/R Pagina   Su/Giù Scegli   A OK   B Indietro   + Esci", "L/R \xe3\x83\x9a\xe3\x83\xbc\xe3\x82\xb8   \xe4\xb8\x8a\xe4\xb8\x8b \xe9\x81\xb8\xe6\x8a\x9e   A \xe6\xb1\xba\xe5\xae\x9a   B \xe6\x88\xbb\xe3\x82\x8b   + \xe7\xb5\x82\xe4\xba\x86"}},
    ...
    {"home.card.desc",  {"Lädt die Datenbank (database.db) und alle Cheat-Dateien (switch-cheats.zip) im Atmosphère-Layout direkt auf die SD-Karte.", "Downloads the database (database.db) and every cheat file (switch-cheats.zip) straight onto the SD card in Atmosphère layout.", "Descarga la base de datos (database.db) y todos los archivos de trucos (switch-cheats.zip) a la SD con el formato de Atmosphère.", "Télécharge la base (database.db) et tous les fichiers de cheats (switch-cheats.zip) sur la carte SD au format Atmosphère.", "Scarica il database (database.db) e tutti i file di trucchi (switch-cheats.zip) sulla SD nel formato Atmosphère.", "データベース(database.db)と全チートファイル(switch-cheats.zip)をAtmosphère形式でSDカードに直接保存します。"}},
    ...
    {"src.gbatemp.name",  {"GBAtemp-Archiv", "GBAtemp archive", "Archivo GBAtemp", "Archive GBAtemp", "Archivio GBAtemp", "GBAtempアーカイブ"}},
    {"src.gbatemp.desc",  {"Das große GBAtemp-Cheat-Archiv (contents_complete.zip).", "The big GBAtemp cheat archive (contents_complete.zip).", "El gran archivo de trucos de GBAtemp.", "La grande archive de cheats GBAtemp.", "Il grande archivio di trucchi GBAtemp.", "GBAtempの大型チートアーカイブ。"}},
    {"src.ibnux.name",    {"Ibnux switch-cheat", "Ibnux switch-cheat", "Ibnux switch-cheat", "Ibnux switch-cheat", "Ibnux switch-cheat", "Ibnux switch-cheat"}},
    {"src.ibnux.desc",    {"ibnux/switch-cheat (Live-Repo, master-Branch).", "ibnux/switch-cheat (live repo, master branch).", "ibnux/switch-cheat (repo en vivo).", "ibnux/switch-cheat (dépôt live).", "ibnux/switch-cheat (repo live).", "ibnux/switch-cheat（ライブリポジトリ）。"}},
    {"src.chansey.name",  {"Chansey 60FPS/Res/GFX", "Chansey 60FPS/Res/GFX", "Chansey 60FPS/Res/GFX", "Chansey 60FPS/Res/GFX", "Chansey 60FPS/Res/GFX", "Chansey 60FPS/Res/GFX"}},
    {"src.chansey.desc",  {"Die Original-Quelle der 60-FPS-/Grafik-Cheats (Live-Repo).", "The original source of the 60 FPS / graphics cheats (live repo).", "La fuente original de los trucos 60 FPS/gráficos.", "La source originale des cheats 60 FPS/graphismes.", "La fonte originale dei trucchi 60 FPS/grafica.", "60FPS/グラフィックチートの元ソース（ライブ）。"}},
    {"src.mynxcheats.name", {"MyNXCheats", "MyNXCheats", "MyNXCheats", "MyNXCheats", "MyNXCheats", "MyNXCheats"}},
    {"src.mynxcheats.desc", {"Kuratiert für ~50 aktuelle Top-Titel (TotK, Karmesin/Purpur …).", "Curated for ~50 recent big titles (TotK, Scarlet/Violet …).", "Selección de ~50 títulos recientes.", "Sélection de ~50 titres récents.", "Selezione di ~50 titoli recenti.", "最近の主要50タイトルを厳選（TotK等）。"}},
    ...
    // ------------------------------------------------------------------
    // System-Seite: installierte Spiele + laufendes Spiel + Saves (v2.1)
    // ------------------------------------------------------------------
    {"nav.system",     {"System", "System", "Sistema", "Système", "Sistema", "システム"}},
    {"eyebrow.system", {"Konsole", "Console", "Consola", "Console", "Console", "本体"}},
    {"sub.system",     {"Installierte Spiele, laufendes Spiel und Speicherstände.", "Installed games, running game and save data.", "Juegos instalados, juego en ejecución y partidas.", "Jeux installés, jeu en cours et sauvegardes.", "Giochi installati, gioco in esecuzione e salvataggi.", "インストール済みゲーム・実行中のゲーム・セーブデータ。"}},
    {"nav.saves",      {"Speicherstände", "Saves", "Partidas", "Sauvegardes", "Salvataggi", "セーブ"}},
    {"eyebrow.saves",  {"Speicher", "Saves", "Partidas", "Sauvegardes", "Salvataggi", "セーブ"}},
    {"sub.saves",      {"Wähle ein Spiel, um seine Speicherstände zu sichern oder wiederherzustellen.", "Pick a game to back up or restore its save data.", "Elige un juego para copiar o restaurar sus partidas.", "Choisissez un jeu pour sauvegarder ou restaurer ses données.", "Scegli un gioco per salvare o ripristinare i suoi salvataggi.", "ゲームを選んでセーブをバックアップ／復元します。"}},
    {"footer.savegames", {"A Speicherstände verwalten   Y Scannen   B Zurück   L/R Seite", "A Manage saves   Y Scan   B Back   L/R Page", "A Gestionar partidas   Y Escanear   B Volver   L/R Página", "A Gérer sauvegardes   Y Analyser   B Retour   L/R Page", "A Gestisci salvataggi   Y Scansiona   B Indietro   L/R Pagina", "A セーブ管理   Y スキャン   B 戻る   L/R ページ"}},

    {"sys.scanning",        {"Scanne Konsole...", "Scanning console...", "Escaneando la consola...", "Analyse de la console...", "Scansione console...", "本体をスキャン中..."}},
    {"sys.scandone.prefix", {"Scan fertig: ", "Scan complete: ", "Escaneo completo: ", "Analyse terminée : ", "Scansione completata: ", "スキャン完了: "}},
    {"sys.scandone.suffix", {" Spiele.", " games.", " juegos.", " jeux.", " giochi.", " 件のゲーム。"}},
    {"sys.running",         {"Laufendes Spiel", "Running game", "Juego en ejecución", "Jeu en cours", "Gioco in esecuzione", "実行中のゲーム"}},
    {"sys.tid",             {"Title-ID", "Title ID", "Title ID", "Title ID", "Title ID", "タイトルID"}},
    {"sys.ver",             {"Version", "Version", "Versión", "Version", "Versione", "バージョン"}},
    {"sys.bid",             {"Build-ID", "Build ID", "Build ID", "Build ID", "Build ID", "ビルドID"}},
    {"sys.bid.unknown",     {"(nur bei laufendem Spiel)", "(only while a game runs)", "(solo con un juego activo)", "(seulement pendant un jeu)", "(solo con un gioco attivo)", "（ゲーム実行中のみ）"}},
    {"sys.bids",            {"Build-IDs", "Build IDs", "Build IDs", "Build IDs", "Build ID", "ビルドID"}},
    {"sys.bid.live",        {"Läuft:", "Running:", "En ejecución:", "En cours :", "In esecuzione:", "実行中:"}},
    {"sys.bid.none",        {"Keine in der DB — Build-ID nur bei laufendem Spiel auslesbar.", "None in the database — build ID only readable while the game runs.", "Ninguna en la base — el build ID solo se lee con el juego en marcha.", "Aucune en base — build ID lisible seulement pendant le jeu.", "Nessuna nel DB — build ID leggibile solo col gioco attivo.", "DBになし — ビルドIDは実行中のみ取得可能。"}},
    {"sys.bid.more",        {"weitere", "more", "más", "de plus", "altri", "その他"}},
    {"sys.cheats.yes",      {"Cheats vorhanden", "Cheats available", "Trucos disponibles", "Cheats disponibles", "Trucchi disponibili", "チートあり"}},
    {"sys.cheats.no",       {"keine Cheats in DB", "no cheats in DB", "sin trucos en la base", "aucun cheat en base", "nessun trucco nel DB", "DBにチートなし"}},
    {"sys.open.zr",         {"ZR: Cheats öffnen", "ZR: open cheats", "ZR: abrir trucos", "ZR : ouvrir cheats", "ZR: apri trucchi", "ZR: チートを開く"}},
    {"sys.norunning",       {"Kein Spiel aktiv", "No game running", "Ningún juego activo", "Aucun jeu en cours", "Nessun gioco attivo", "実行中のゲームなし"}},
    {"sys.norunning.hint",  {"Starte ein Spiel und wechsle per Forwarder zurück.", "Start a game and switch back via the forwarder.", "Inicia un juego y vuelve con el forwarder.", "Lance un jeu et reviens via le forwarder.", "Avvia un gioco e torna col forwarder.", "ゲームを起動し、フォワーダーで戻ってください。"}},
    {"sys.installed",       {"Installierte Spiele", "Installed games", "Juegos instalados", "Jeux installés", "Giochi installati", "インストール済みゲーム"}},
    {"sys.rescan",          {"Y: neu scannen", "Y: rescan", "Y: reescanear", "Y : réanalyser", "Y: riscansiona", "Y: 再スキャン"}},
    {"sys.scanhint",        {"Y: scannen", "Y: scan", "Y: escanear", "Y : analyser", "Y: scansiona", "Y: スキャン"}},
    {"sys.empty",           {"Keine Spiele gefunden (Y zum Scannen).", "No games found (press Y to scan).", "No se encontraron juegos (pulsa Y).", "Aucun jeu trouvé (appuyez sur Y).", "Nessun gioco trovato (premi Y).", "ゲームが見つかりません（Yでスキャン）。"}},
    {"app.loading",         {"Lädt…", "Loading…", "Cargando…", "Chargement…", "Caricamento…", "読み込み中…"}},
    {"app.loadingdb",       {"Lade Datenbank", "Loading database", "Cargando base de datos", "Chargement de la base", "Caricamento database", "データベース読み込み中"}},
    {"sys.appmode.title",   {"System-Funktionen brauchen den Application-Modus", "System features need Application mode", "Las funciones del sistema requieren el modo Aplicación", "Les fonctions système nécessitent le mode Application", "Le funzioni di sistema richiedono la modalità Applicazione", "システム機能にはアプリケーションモードが必要です"}},
    {"sys.appmode.hint",    {"Installierte Spiele und Speicherstände sind nur zugänglich, wenn die App als vollwertige Anwendung läuft (wie Checkpoint/JKSV). Starte sie über das Album, während du R auf einem Spiel gedrückt hältst – oder installiere einen Forwarder. Details stehen im Protokoll.", "Installed games and saves are only accessible when the app runs as a full application (like Checkpoint/JKSV). Launch it from the Album while holding R on a game — or install a forwarder. See the Log page for details.", "Los juegos instalados y las partidas solo son accesibles cuando la app se ejecuta como aplicación completa (como Checkpoint/JKSV). Ábrela desde el Álbum manteniendo R sobre un juego, o instala un forwarder. Consulta el Registro para más detalles.", "Les jeux installés et les sauvegardes ne sont accessibles que si l'app s'exécute comme une vraie application (comme Checkpoint/JKSV). Lancez-la depuis l'Album en maintenant R sur un jeu, ou installez un forwarder. Voir le Journal pour les détails.", "I giochi installati e i salvataggi sono accessibili solo se l'app è eseguita come applicazione completa (come Checkpoint/JKSV). Avviala dall'Album tenendo premuto R su un gioco, o installa un forwarder. Vedi il Registro per i dettagli.", "インストール済みゲームとセーブは、アプリが完全なアプリケーションとして実行されている場合のみ利用できます（Checkpoint/JKSVと同様）。アルバムからゲーム上でRを押しながら起動するか、フォワーダーをインストールしてください。詳細はログをご覧ください。"}},
    {"sys.hascheats",       {"Cheats", "Cheats", "Trucos", "Cheats", "Trucchi", "チート"}},
    {"sys.nocheats",        {"Keine Cheats für dieses Spiel in der Datenbank.", "No cheats for this game in the database.", "No hay trucos para este juego en la base.", "Aucun cheat pour ce jeu dans la base.", "Nessun trucco per questo gioco nel database.", "このゲームのチートはDBにありません。"}},

    {"sys.detail.eyebrow",  {"System / Spiel", "System / Game", "Sistema / Juego", "Système / Jeu", "Sistema / Gioco", "システム / ゲーム"}},
    {"sys.author",          {"Herausgeber", "Publisher", "Editor", "Éditeur", "Editore", "発売元"}},
    {"sys.db",              {"In unserer DB", "In our database", "En nuestra base", "Dans notre base", "Nel nostro DB", "当DB内"}},
    {"sys.db.yes",          {"Ja — ZR öffnet Cheats", "Yes — ZR opens cheats", "Sí — ZR abre trucos", "Oui — ZR ouvre les cheats", "Sì — ZR apre i trucchi", "あり — ZRでチート"}},
    {"sys.db.no",           {"Nein", "No", "No", "Non", "No", "なし"}},
    {"sys.saves",           {"Speicherstände", "Save data", "Partidas guardadas", "Sauvegardes", "Salvataggi", "セーブデータ"}},
    {"sys.saves.users",     {"Nutzer", "Users", "Usuarios", "Utilisateurs", "Utenti", "ユーザー"}},
    {"sys.saves.backups",   {"Backups", "Backups", "Copias", "Sauvegardes", "Backup", "バックアップ"}},
    {"sys.saves.enter",     {"A: Speicherstände verwalten (Backup / Restore)", "A: manage save data (backup / restore)", "A: gestionar partidas (copia / restaurar)", "A : gérer les sauvegardes (backup / restauration)", "A: gestisci salvataggi (backup / ripristino)", "A: セーブ管理（バックアップ / 復元）"}},

    {"sys.saves.eyebrow",         {"Speicherstände", "Save data", "Partidas guardadas", "Sauvegardes", "Salvataggi", "セーブデータ"}},
    {"sys.saves.none",            {"Kein Speicherstand für dieses Spiel.", "No save data for this game.", "No hay partidas para este juego.", "Aucune sauvegarde pour ce jeu.", "Nessun salvataggio per questo gioco.", "このゲームのセーブはありません。"}},
    {"sys.saves.commonuser",      {"Gerät", "Device", "Dispositivo", "Appareil", "Dispositivo", "本体"}},
    {"sys.saves.backup",          {"Backup erstellen", "Create backup", "Crear copia", "Créer une sauvegarde", "Crea backup", "バックアップ作成"}},
    {"sys.saves.backup.hint",     {"A: Speicherstand dieses Nutzers sichern", "A: back up this user's save", "A: copiar la partida de este usuario", "A : sauvegarder la partie de cet utilisateur", "A: salva i dati di questo utente", "A: このユーザーのセーブを保存"}},
    {"sys.saves.restore.hint",    {"A: wiederherstellen   X: löschen", "A: restore   X: delete", "A: restaurar   X: borrar", "A : restaurer   X : supprimer", "A: ripristina   X: elimina", "A: 復元   X: 削除"}},
    {"sys.saves.restore.confirm", {"Nochmal A: Speicherstand WIRKLICH überschreiben", "Press A again: REALLY overwrite save", "Pulsa A otra vez: SOBRESCRIBIR la partida", "A à nouveau : ÉCRASER la sauvegarde", "A di nuovo: SOVRASCRIVERE il salvataggio", "もう一度A: セーブを本当に上書き"}},

    {"save.backing",      {"Erstelle Backup...", "Creating backup...", "Creando copia...", "Création de la sauvegarde...", "Creazione backup...", "バックアップ作成中..."}},
    {"save.file",         {"Datei:", "File:", "Archivo:", "Fichier :", "File:", "ファイル:"}},
    {"save.backup.ok",    {"Backup erstellt.", "Backup created.", "Copia creada.", "Sauvegarde créée.", "Backup creato.", "バックアップを作成しました。"}},
    {"save.backup.fail",  {"Backup fehlgeschlagen", "Backup failed", "Fallo al copiar", "Échec de la sauvegarde", "Backup non riuscito", "バックアップに失敗"}},
    {"save.restoring",    {"Stelle wieder her...", "Restoring...", "Restaurando...", "Restauration...", "Ripristino...", "復元中..."}},
    {"save.restore.ok",   {"Speicherstand wiederhergestellt.", "Save restored.", "Partida restaurada.", "Sauvegarde restaurée.", "Salvataggio ripristinato.", "セーブを復元しました。"}},
    {"save.restore.fail", {"Wiederherstellung fehlgeschlagen", "Restore failed", "Fallo al restaurar", "Échec de la restauration", "Ripristino non riuscito", "復元に失敗"}},
    {"save.deleted",      {"Backup gelöscht.", "Backup deleted.", "Copia borrada.", "Sauvegarde supprimée.", "Backup eliminato.", "バックアップを削除しました。"}},

    {"footer.system",    {"A Details   ZR Cheats (laufend)   Y Scannen   L/R Seite", "A Details   ZR Cheats (running)   Y Scan   L/R Page", "A Detalles   ZR Trucos   Y Escanear   L/R Página", "A Détails   ZR Cheats   Y Analyser   L/R Page", "A Dettagli   ZR Trucchi   Y Scansiona   L/R Pagina", "A 詳細   ZR チート   Y スキャン   L/R ページ"}},
    {"footer.sysdetail", {"A Speicherstände   ZR Cheats   B Zurück", "A Save data   ZR Cheats   B Back", "A Partidas   ZR Trucos   B Volver", "A Sauvegardes   ZR Cheats   B Retour", "A Salvataggi   ZR Trucchi   B Indietro", "A セーブ   ZR チート   B 戻る"}},
    {"footer.saves",     {"A Backup/Restore   X Löschen   B Zurück", "A Backup/Restore   X Delete   B Back", "A Copia/Restaurar   X Borrar   B Volver", "A Backup/Restaurer   X Supprimer   B Retour", "A Backup/Ripristina   X Elimina   B Indietro", "A バックアップ/復元   X 削除   B 戻る"}},
};
// clang-format on
